        ...
        body: StreamBuilder(
          ...
          builder: (BuildContext context, AsyncSnapshot<User?> snapshot) {
            return Column(
              children: [
                Center(
                  child: Text(snapshot.data == null ? "Logged out" : "Logged In"),
                ),
                if (snapshot.data == null || snapshot.data?.isAnonymous == true) ...[
                  ElevatedButton(
                    onPressed: () async {
                      await performLogin("apple.com", ["email"], {"locale": "en"});
                    },
                    child: Text("Sign in By Apple"),
                  ),
                  ElevatedButton(
                    onPressed: () async {
                      await performLogin("twitter.com", ["email"], {"lang": "en"});
                    },
                ...
                if (snapshot.data != null) ...[
                  ElevatedButton(
                    onPressed: () async {
                      await performLink("apple.com", ["email"], {"locale": "en"});
                    },
                    child: Text("Link Sign in By Apple"),
                  ),
                  ElevatedButton(
                    onPressed: () async {
                      await performLink("twitter.com", ["email"], {"lang": "en"});
                    },
                    child: Text("Link Sign in By Twitter"),
                  ),
                  ElevatedButton(
                    onPressed: () async {
                      await performLink("github.com", ["user:email"], {"lang": "en"});
                    },
